...
def sdf_vectorized(double t, np.ndarray x,
    ...
    cdef np.ndarray[np.float64_t, ndim=1] r = np.array([0.04445,0.04445,0.04445,0.04445,0.04445,0.04445,0.04445,0.04445,0.04445,0.04445,0.05715,0.05715,0.05715,0.05715,0.05715,0.05715])
    ...
    for i in range(n):
        ...
        for ti in range(len(tx)):
            ...
            Y_0 = x[i,1] - ty[ti]
            Z_0 = x[i,2]
            phi[i] = min(phi[i], sqrt(X_0**2 + Y_0**2) - r[ti])  #0.02721428571428571)
#            for ri in range(nr):
